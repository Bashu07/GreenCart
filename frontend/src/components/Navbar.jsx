...
const Navbar = () => {
    ...
    const { user, setUser, setShowUserLogin, navigate, searchQuery, setSearchQuery, getCartAmount, axios, getCartCount, } = useAppContext(AppContext)
    ...
    return (
        <nav className="flex items-center justify-between px-6 md:px-16 lg:px-24 xl:px-32 py-4 border-b border-gray-300 bg-white relative transition-all">
            ...
            <div className="hidden sm:flex items-center gap-8">
                <NavLink
                    to="/seller"
                    className="border border-dotted border-primary text-primary px-4 py-2 rounded-md hover:bg-primary hover:text-white transition duration-300"
                >
                    Seller Dashboard
                </NavLink>
                    ...
                    !user ? (
                        ...
                        (
                            <div className='relative group '>
                                <img src={assets.profile_icon} className='w-10 rounded-full cursor-pointer border border-gray-300' alt="" />
                                <ul className='hidden group-hover:block absolute top-10 right-0 bg-white shadow border border-gray-200 py-2 w-40 rounded-md text-sm z-40'>
                                    ...
                                    <li className="p-1.5 hover:bg-primary/10 cursor-pointer " onClick={logout}>Logout</li>
                                </ul>
                            </div>
                        )
                }
